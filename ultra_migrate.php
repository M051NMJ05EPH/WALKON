    ...
    foreach ($products as $p) {
        ...
        if (!empty($p['sizes'])) {
            ...
            foreach ($sizes as $size) {
                if ($size) $pdo->prepare("INSERT INTO product_sizes (product_id, size_value) VALUES (?, ?)")->execute([$new_id, $size]);
            }
        } elseif (!empty($p['size'])) { // check singular
             $pdo->prepare("INSERT INTO product_sizes (product_id, size_value) VALUES (?, ?)")->execute([$new_id, $p['size']]);
        }
        ...
        if (!empty($p['colors'])) {
            ...
            foreach ($colors as $color) {
                if ($color) $pdo->prepare("INSERT INTO product_colors (product_id, color_name) VALUES (?, ?)")->execute([$new_id, $color]);
            }
        } elseif (!empty($p['color'])) { // check singular
            $pdo->prepare("INSERT INTO product_colors (product_id, color_name) VALUES (?, ?)")->execute([$new_id, $p['color']]);
        }

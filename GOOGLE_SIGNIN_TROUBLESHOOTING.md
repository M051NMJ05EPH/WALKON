...
Ensure your `config.php` contains the correct Google Client ID and Secret. Since the authentication system is now consolidated, all sign-in attempts correctly flow through `google-login.php`.
...
4. Under **Authorized redirect URIs**, add:
   ```
   http://localhost/WALKON-rough/google-callback.php
   ```
5. Click **Save**
...
- [ ] Client ID and Secret in `config.php` are correct
...
- ✓ After authorization, you're redirected to `google-verify.php`
- ✓ Click "Continue" to be redirected to `dashboard.php`
- ✓ New user appears in database with `google_id` populated
- ✓ You're logged in (session variables set: `user_id`, `email`, `first_name`, `last_name`)
- ✓ Can access protected pages like `dashboard.php`

---

**Need more help?** Check the Apache error logs or the browser console for specific error messages.
